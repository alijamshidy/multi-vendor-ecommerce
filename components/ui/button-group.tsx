  ...
  "group/button-group flex w-fit items-stretch *:focus-visible:relative *:focus-visible:z-10 has-[>[data-slot=button-group]]:gap-2 has-[select[aria-hidden=true]:last-child]:[&>[data-slot=select-trigger]:last-of-type]:rounded-e-lg [&>[data-slot=select-trigger]:not([class*='w-'])]:w-fit [&>input]:flex-1",
  {
    variants: {
      orientation: {
        ...
        vertical:
          "flex-col [&>*:not(:first-child)]:rounded-bs-none [&>*:not(:first-child)]:border-bs-0 [&>*:not(:last-child)]:rounded-be-none [&>[data-slot]:not(:has(~[data-slot]))]:rounded-be-lg!",
      },
    },
    defaultVariants: {
...
function ButtonGroupText({
  ...
  return (
    <Comp
      className={cn(
        "flex min-w-9 items-center justify-center gap-2 rounded-lg border bg-muted px-2.5 text-sm font-medium tabular-nums in-data-[slot=button-group]:rounded-none [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />

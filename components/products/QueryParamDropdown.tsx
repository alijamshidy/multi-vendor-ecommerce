...
export default function QueryParamDropdown({
  ...
  const handleChange = useDebouncedCallback((index: number) => {
    const nextValue = optionValues?.[index] ?? options[index];
    setDisplayValue(options[index] ?? nextValue);
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent
          ...
          {options.map((option, index) => (
            <Fragment key={String(optionValues?.[index] ?? option)}>
              <DropdownMenuItem onClick={() => handleChange(index)}>
                <span>{option}</span>
              </DropdownMenuItem>
              {index < options.length - 1 && (
                <DropdownMenuSeparator
                  key={`separator-${String(optionValues?.[index] ?? option)}`}
                />
              )}
            </Fragment>
          ))}
